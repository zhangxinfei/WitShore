...
public class JsonUtil {
     ...
     *  集合中包含对象 对象中包含另一个集合
     * @param JSON 需要转换的集合
     * @param elementClass 集合中的对象类型
     * @return 集合
     *
     */
    public static<T> T CollectionString2Obj(String JSON,Class<?> elementClass){
        JavaType javaType4 = objectMapper.getTypeFactory().constructParametricType(List.class,elementClass);
        try {
            return objectMapper.readValue(JSON, javaType4);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
